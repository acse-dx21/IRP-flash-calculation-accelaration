...
def model_cv(**kwargs):
    ...
    loss={"target":[]}
    data_saved_root = saved_root + "continues_pred" + os.sep + "mix_2" + os.sep + "mixture_norm.csv"
    for i in range(1000):
        pred = model_instance.predict(X_test)
        pred=normalization(pred)
        loss["target"].append(mean_squared_error(pred,y_test))
        X_test[...,-2:]=compute_material_fraction(pred,2)

    pd.DataFrame(loss).to_csv(data_saved_root)
