...
def model_cv(**kwargs):
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if progress.fail or epoch == max_epochs:
            data_record["trainning_time_consume(s)"].append(time.time() - start_train)
            ...
            test_score = evaluate(X['test'], y['test'])
            data_record["test_time_consume(s)"].append(time.time() - start_test)
            ...
            break
    data_record["epochs"].append(epoch)

    epochs = 1500
    loss_data = {"target": []}
    loss_sum=0
    cnt = 0
    print("train_shape", X['train'].shape, "X_test_shape", X['test'].shape)

    #contunues predition
    if True:
        conti_data_saved_root = saved_root + "continues_pred" + os.sep + "mix_2" + os.sep + f"mixture_experience{cnt}.csv"

        X_test_conti = np.copy(preprocess.inverse_transform(X['test'].cpu().detach().numpy()))
        y_test_conti = torch.clone(y['test']).to(device)  # to make code look better, we copy it too.
        # try:
        if True:
            for i in range(epochs):
                data_loader = zero.data.IndexLoader(len(X_test_conti), batch_size)
                for iteration, batch_idx in enumerate(data_loader):
                    preprocess.transform(X_test_conti[batch_idx])
                    batch_data=torch.Tensor(preprocess.transform(X_test_conti[batch_idx])).to(device)
                    pred = model(batch_data,None)

                    loss_single = nn.MSELoss()(pred, y_test_conti[batch_idx])*len(batch_data)
                    loss_sum+=loss_single.cpu().item()
                    X_test_conti[batch_idx][..., -2:] = compute_material_fraction(pred.cpu().detach().numpy(), 2)
                print(i, loss_sum/len(X_test_conti))
                print("----------------X----------------", )
                print(X_test_conti)
                print("---------------------------------", )
                loss_data["target"].append(loss_sum/len(X_test_conti))
...
def run_bayes_optimize(num_of_iteration=10, data_index=2):
    ...
    print(X_test)
    print(preprocess.inverse_transform(X_test))
